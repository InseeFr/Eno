...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:eno="http://xml.insee.fr/apps/eno" xmlns:enofo="http://xml.insee.fr/apps/eno/out/form-runner" xmlns:fox="http://xmlgraphics.apache.org/fop/extensions" exclude-result-prefixes="xd eno enofo" version="2.0">
            ...
            <xsl:when test="$context='business'">
                <xsl:choose>
                    <xsl:when test="$comment-question and $response-time-question">
                        <xsl:apply-templates select="$end-question//fo:page-sequence/fo:flow/*[not(@id='COMMENT_QUESTION_TITLE' or @id='TIME_QUESTION_TITLE')]"/>
                    </xsl:when>
                    <xsl:when test="$comment-question">
                        <xsl:apply-templates select="$end-question//fo:page-sequence/fo:flow/*[@id='COMMENT_QUESTION_TITLE' or @id='COMMENT_QUESTION']"/>
                    </xsl:when>
                    <xsl:when test="$response-time-question">
                        <xsl:apply-templates select="$end-question//fo:page-sequence/fo:flow/*[@id='TIME_QUESTION_TITLE' or @id='TIME_QUESTION']"/>
                    </xsl:when>
                </xsl:choose>
            </xsl:when>
            <xsl:otherwise>
                <xsl:if test="$comment-question">
                    <xsl:apply-templates select="$end-question//fo:page-sequence/fo:flow/*"/>
                </xsl:if>
            </xsl:otherwise>
        </xsl:choose>
        <xsl:copy-of select="."/>
