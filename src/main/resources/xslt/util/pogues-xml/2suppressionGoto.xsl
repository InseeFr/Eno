...
<xsl:stylesheet 
    ...
    <xd:doc>
        every attributes and tags not containing "id"
        <xd:desc/>
    </xd:doc>
    ...
    <xsl:template match="pogues:GoTo">
        <xsl:copy-of select="."/>
    </xsl:template>
    ...
    <xsl:template name="find_id_node_without_child">
        ...
        <xsl:copy>
            ...
            <xsl:if test="count($temp0/*) ne 0">
                <poguesGoto:GoToExpressions>
                    <xsl:copy-of select="$temp0"></xsl:copy-of>
                </poguesGoto:GoToExpressions>
            </xsl:if>
    ...
    <xsl:template name="build_child">
        <xsl:param name="goto_expressions"/>
            ...
            <xsl:choose>
                <xsl:when test="count($goto_expressions/poguesGoto:Expression)=0">
                    <xsl:copy-of select="./poguesGoto:GoToExpressions/poguesGoto:Expression"/>
                </xsl:when>
                <xsl:otherwise>
                    <!--xsl:message><xsl:value-of select="./@id"/></xsl:message-->
                    <xsl:for-each select="./poguesGoto:GoToExpressions/poguesGoto:Expression">
                        <xsl:variable name="expr_current" ><xsl:value-of select="current()"/></xsl:variable>
        ...
        <xsl:copy>
            ...
            <xsl:if test="count($gotoChild/*) gt 0">
                <poguesGoto:GoToExpressions>
                    <xsl:copy-of select="$gotoChild"></xsl:copy-of>
                </poguesGoto:GoToExpressions>
            </xsl:if>
    ...
    <xsl:template name="find_intersection">
        ...
        <xsl:choose>
            ...
            <xsl:when test="count($remaining_list/*)=0">
                <xsl:copy-of select="$prior_value"></xsl:copy-of>
            </xsl:when>
            <xsl:otherwise>
                <xsl:call-template name="find_intersection">
                    <xsl:with-param name="prior_value">
