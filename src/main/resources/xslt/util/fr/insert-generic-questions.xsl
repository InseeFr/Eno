...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="begin-questions-identification" as="xs:boolean"
        select="$parameters//fr-parameters/BeginQuestion/Identification"/>
    <xsl:variable name="end-response-time-question" as="xs:boolean">
        <xsl:choose>
            <xsl:when test="$parameters//fr-parameters/EndQuestion/ResponseTimeQuestion != ''">
                <xsl:value-of select="$parameters//fr-parameters/EndQuestion/ResponseTimeQuestion"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="$properties//fr-parameters/EndQuestion/ResponseTimeQuestion"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
    <xsl:variable name="end-comment-question" as="xs:boolean">
        <xsl:choose>
            <xsl:when test="$parameters//fr-parameters/EndQuestion/CommentQuestion != ''">
                <xsl:value-of select="$parameters//fr-parameters/EndQuestion/CommentQuestion"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="$properties//fr-parameters/EndQuestion/CommentQuestion"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
                        ...
                        <fr:section id="INSEE-BUSINESS-TEMPS-control" bind="INSEE-BUSINESS-TEMPS-bind" name="INSEE-BUSINESS-TEMPS">
                            ...
                            <xsl:if test="$end-response-time-question">
                                <xhtml:div class="question">
                                    <xf:output id="INSEE-BUSINESS-TEMPS-QI-1-control" name="INSEE-BUSINESS-TEMPS-QI-1" bind="INSEE-BUSINESS-TEMPS-QI-1-bind"
                ...
                <xsl:copy>
                    ...
                    <xsl:if test="$end-response-time-question or $end-comment-question">
                        <fr:section id="INSEE-HOUSEHOLD-TEMPS-control" bind="INSEE-HOUSEHOLD-TEMPS-bind" name="INSEE-HOUSEHOLD-TEMPS">
                            ...
                            <xsl:if test="$end-response-time-question">
                                <xhtml:div class="question">
                                    <xf:output id="INSEE-HOUSEHOLD-TEMPS-QI-1-control" name="INSEE-HOUSEHOLD-TEMPS-QI-1" bind="INSEE-HOUSEHOLD-TEMPS-QI-1-bind"
                                        class="question" xxf:order="label control">
