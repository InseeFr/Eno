...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="ArticleServiceProducteur"
        select="if($metadonnee/InformationsCollecte/ServiceProducteur/Article/text()='l''')
        then($metadonnee/InformationsCollecte/ServiceProducteur/Article)
        else (concat($metadonnee/InformationsCollecte/ServiceProducteur/Article/text(),' '))"/>
    <xsl:variable name="LibelleServiceProducteur"
        select="concat($ArticleServiceProducteur,$metadonnee/InformationsCollecte/ServiceProducteur/Libelle)"/>
