...
-->    <!-- La liste des fréquences qui font afficher la période, séparées par des virgules -->
    ...
    <xsl:variable name="URLNotice" select="$metadonnee/InformationsCollecte/Enquete/URLNotice"/>
    <xsl:variable name="URLSpecimen" select="$metadonnee/InformationsCollecte/Enquete/URLSpecimen"/>
    <xsl:variable name="URLDiffusion" select="$metadonnee/InformationsCollecte/Enquete/URLDiffusion"/>
    ...
    <xsl:template match="xf:submission[@id='save' or @id='submit']">
        <xsl:copy>
            ...
            <xsl:variable name="resource">
                <xsl:value-of
                    select="concat('{xxf:property(''server-exist-orbeon'')}/restxq/{xxf:property(''enregistrer-service'')}/',$campagne-value,'/',$modele,'/{xxf:get-request-parameter(''unite-enquete'')}?ongletproof=oui')"
                />
            </xsl:variable>
    ...
    <xsl:template
        ...
        <xf:submission id="expedierPdf" method="post" replace="none">
            <xsl:variable name="resource">
                <xsl:value-of
                    select="concat('/expedier/',$campagne-value,'/{xxf:get-request-parameter(''unite-enquete'')}?modele=',$modele)"
                />
            </xsl:variable>
            <xsl:attribute name="resource" select="$resource"/>
        </xf:submission>
