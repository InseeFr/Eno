...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="accompanying-mail"/>
    
    <xsl:variable name="static-pages" select="doc('static-pages.fo')"/>
    ...
    <xd:doc>
        <xd:desc>
            <xd:p>add generic end questions.</xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:template match="fo:block[@id='TheVeryLastPage']">
        <xsl:copy-of select="$static-pages//fo:page-sequence[@master-reference='end-questions']/fo:flow/fo:block"/>
        <xsl:copy>
            <xsl:apply-templates select="node() | @*"/>
        </xsl:copy>
    </xsl:template>
