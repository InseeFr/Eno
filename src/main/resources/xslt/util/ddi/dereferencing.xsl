...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:key name="loop-variablegroup" match="/ddi-instance:DDIInstance/g:ResourcePackage/l:VariableScheme/l:VariableGroup[l:TypeOfVariableGroup='Loop']" use="r:BasedOnObject/r:BasedOnReference[1]/r:ID"/>
    ...
    <xsl:template match="d:QuestionGrid[d:GridDimension/d:Roster]/d:StructuredMixedGridResponseDomain" mode="output-DDI">
        ...
        <xsl:copy>
            <xsl:apply-templates select="key('loop-variablegroup',$loop-ID)/r:VariableReference/key('external-variable',r:ID)" mode="output-DDI"/>
            <xsl:apply-templates select="*" mode="output-DDI"/>
