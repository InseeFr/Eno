   ...
   <xsl:template match="d:Instruction[descendant::d:ConditionalText]"
      ...
      <xsl:variable name="result">
         ...
         <xsl:for-each select="tokenize($text,'ø')[not(.='')]">
            <xsl:text>,</xsl:text>
            <xsl:choose><!-- If the split string has a match in the following elements, it means it is a dynamic part of the text, it will be in the instance of the generated xforms -->
               <xsl:when test=".=$condition/d:ConditionalText/r:SourceParameterReference/r:OutParameter/r:ID/text()                         or contains($condition/d:ConditionalText/d:Expression/r:Command/r:CommandContent/text(),.)">
