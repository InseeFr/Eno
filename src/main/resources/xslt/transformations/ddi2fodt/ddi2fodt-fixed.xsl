...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:enoddi2fodt="http://xml.insee.fr/apps/eno/ddi2fodt"
    ...
    <xsl:import href="../../outputs/fodt/models.xsl"/>
    ...
    <xsl:variable name="labels-resource">
        <xsl:sequence select="eno:build-labels-resource($labels-folder,enofodt:get-form-languages(//d:Sequence[d:TypeOfSequence/text()='template']))"/>
    </xsl:variable>
	...
	<xsl:function name="enofodt:get-variable-business-name">
    ...
    <xsl:function name="enofodt:get-form-languages">
        <xsl:param name="context" as="item()"/>
        <xsl:choose>
    ...
    <xsl:function name="enofodt:get-formatted-label">
        <xsl:param name="context" as="item()"/>
        <xsl:param name="language"/>
        <xsl:variable name="tempLabel">
            <xsl:apply-templates select="enoddi:get-label($context,$language)" mode="enofodt:format-label">
    ...
    <xsl:function name="enofodt:get-after-question-title-instructions">
        <xsl:param name="context" as="item()"/>
    ...
    <xsl:function name="enofodt:get-end-question-instructions">
        <xsl:param name="context" as="item()"/>
    ...
    <xsl:template match="*" mode="enofodt:format-label" priority="-1">
        <xsl:copy>
            <xsl:apply-templates select="node()|@*" mode="enofodt:format-label"/>      
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="xhtml:p | xhtml:span" mode="enofodt:format-label">
        <xsl:apply-templates select="node()" mode="enofodt:format-label"/>
    </xsl:template>
    
    <xsl:template match="xhtml:span[@class='block']" mode="enofodt:format-label">
            <xsl:apply-templates select="node()" mode="enofodt:format-label"/>        
    </xsl:template>
        
    <xsl:template match="text()" mode="enofodt:format-label">
    ...
    <xsl:template match="xhtml:i" mode="enofodt:format-label">
        <text:span text:style-name="italic">
            <xsl:apply-templates select="node()" mode="enofodt:format-label"/>
        </text:span>
    </xsl:template>
    
    <xsl:template match="xhtml:b" mode="enofodt:format-label">
        <text:span text:style-name="bold">
            <xsl:apply-templates select="node()" mode="enofodt:format-label"/>
        </text:span>
    </xsl:template>
    
    <xsl:template match="xhtml:span[@style='text-decoration:underline']" mode="enofodt:format-label">
        <text:span text:style-name="underline">
            <xsl:apply-templates select="node()" mode="enofodt:format-label"/>
        </text:span>
    </xsl:template>
    
    <xsl:template match="xhtml:br" mode="enofodt:format-label">
        <xsl:text xml:space="preserve">&#xA;</xsl:text>
    </xsl:template>
    
    <xsl:template match="xhtml:a[contains(@href,'#ftn')]" mode="enofodt:format-label">
        <xsl:apply-templates select="node()" mode="enofodt:format-label"/>
