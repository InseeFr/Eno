...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="d:QuestionGrid" mode="iatddi:get-title-line">
      ...
      <xsl:sequence select="d:GridDimension[@rank='2']//(l:Code[count(ancestor::l:CodeList[r:Label])=$index+number($label-or-no)-1] | l:CodeList/r:Label[count(ancestor::l:CodeList[r:Label])=$index+number($label-or-no)])"/>
   </xsl:template>
   ...
   <xsl:template match="l:Code[max(ancestor::d:GridDimension[@rank='1']//l:Code[not(child::l:Code)]/count(ancestor::l:CodeList | ancestor::l:Code))&gt;1]"
                 ...
                 priority="1"><!-- Getting the depth-level of parents codes -->
      ...
      <xsl:value-of select="max(ancestor::d:GridDimension[@rank='1']//l:Code[not(child::l:Code)]/count(ancestor::l:CodeList | ancestor::l:Code))-number($parents)-number($children)+1"/>
   </xsl:template>
   ...
   <xsl:template match="r:Label[parent::l:CodeList/parent::r:CodeListReference/parent::d:CodeDomain/parent::d:GridDimension[@rank='1']]"
      ...
      <xsl:value-of select="max(ancestor::d:GridDimension[@rank='1']/following-sibling::d:GridDimension[@rank='2']//l:Code[not(child::l:Code)]/count(ancestor::l:CodeList[r:Label]))+1-number($label-or-no)"/>
   </xsl:template>
   ...
   <xsl:template match="*" mode="iatddi:get-computation-items" as="xs:string *">
      ...
      <xsl:for-each select="//d:ComputationItem[contains(r:CommandCode/r:Command/r:CommandContent/text(), $id)]"><!-- The computation Item contains a chain corresponding to the variable. But it might not correspond exactly to this variable. --><!-- Therefore, we modify the condition by deleting the potential false positive (same value followed by a dash or a number) -->
