...
public class XFORMSModeleColtranePostprocessor implements Postprocessor {

	private static final Logger logger = LoggerFactory.getLogger(XFORMSModeleColtranePostprocessor.class);
	...
	public File process(File input, byte[] parametersFile, byte[] metadata, byte[] specificTreatmentXsl, byte[] mapping, String survey) throws Exception {
		File outputForFRFile = new File(input.getParent(),
				...
				Constants.MODELE_COLTRANE_XFORMS_EXTENSION);
		...
		InputStream FO_XSL = Constants.getInputStreamFromPath(Constants.UTIL_XFORMS_MODELE_COLTRANE_XSL);
	...
	public String toString() {
		return PostProcessing.XFORMS_MODELE_COLTRANE.name();
	}
